...
// Create global lib obj and its namespaces
// build processes may have already created an lib obj
...
(function(window, document, ionic) {
  ...
  */
  ionic.DomUtil = {
     ...
     * @alias lib.requestAnimationFrame
     ...
     * @alias lib.animationFrameThrottle
     * @description
...
(function(ionic) {
  ...
  ionic.trigger = ionic.EventController.trigger;//function() { lib.EventController.trigger.apply(lib.EventController.trigger, arguments); };
...
(function(ionic) {
  ...
  // are filled by lib.Gestures.event.determineEventTypes on setup
  ...
  function setup() {
    ...
    // Register all gestures inside lib.Gestures.gestures
  ...
  ionic.Gestures.Instance = function(element, options) {
    ...
    // setup lib.GesturesJS window events and register all gestures
  ...
  ionic.Gestures.event = {
     ...
     * @param   {HTMLElement}   element
     * @param   {String}        eventType        like lib.Gestures.EVENT_MOVE
     * @param   {Function}      handler
     */
     ...
     * determine what we need and set them in the lib.Gestures.EVENT_TYPES constant
     */
    ...
    /**
     * collect event data for lib.Gestures js
     * @param   {HTMLElement}   element
     * @param   {String}        eventType        like lib.Gestures.EVENT_MOVE
     * @param   {Object}        eventData
     */
  ...
  ionic.Gestures.PointerEvent = {
     ...
     * @param   {String}   type             lib.Gestures.EVENT_END
     ...
     * @param   {String}        pointerType     lib.Gestures.POINTER_MOUSE
  ...
  ionic.Gestures.utils = {
     ...
     * @param   {Touch}     touch1
     * @param   {Touch}     touch2
     * @returns {String}    direction constant, like lib.Gestures.DIRECTION_LEFT
     */
    ...
    stopDefaultBrowserBehavior: function stopDefaultBrowserBehavior(element, css_class) {
      ...
      // hammer.js doesn't come with CSS, but lib does, which is why we prefer this method
  ...
  ionic.Gestures.detection = {
    // contains all registred lib.Gestures.gestures in the correct order
    gestures: [],

    // data of the current lib.Gestures.gesture detection session
    current: null,

    // the previous lib.Gestures.gesture session data
    ...
    /**
     * start lib.Gestures.gesture detection
     * @param   {ionic.Gestures.Instance}   inst
     * @param   {Object}            eventData
     */
    startDetect: function startDetect(inst, eventData) {
      // already busy with a lib.Gestures.gesture detection on an element
      if(this.current) {
        return;
      }
      ...
      this.current = {
        inst: inst, // reference to lib.GesturesInstance we're working for
    ...
    /**
     * lib.Gestures.gesture detection
     * @param   {Object}    eventData
     */
    detect: function detect(eventData) {
      ...
      // call lib.Gestures.gesture handlers
    ...
    /**
     * clear the lib.Gestures.gesture vars
     * this is called on endDetect, but can also be used when a final lib.Gestures.gesture has been detected
     * to stop other lib.Gestures.gestures from being fired
     */
    ...
    /**
     * extend eventData for lib.Gestures.gestures
     * @param   {Object}   ev
     * @returns {Object}   ev
     */
    ...
    register: function register(gesture) {
      ...
      // extend lib.Gestures default options with the lib.Gestures.gesture options
      ionic.Gestures.utils.extend(ionic.Gestures.defaults, options, true);

      // set its index
      gesture.index = gesture.index || 1000;

      // add lib.Gestures.gesture to the list
   ...
   *          pointerType {String}        kind of pointer that was used. matches lib.Gestures.POINTER_MOUSE|TOUCH
   ...
   *          direction   {String}        the direction we are moving. matches lib.Gestures.DIRECTION_UP|DOWN|LEFT|RIGHT
   *          distance    {Number}        the distance we haved moved
   *          scale       {Number}        scaling of the touches, needs 2 touches
   *          rotation    {Number}        rotation of the touches, needs 2 touches *
   *          eventType   {String}        matches lib.Gestures.EVENT_START|MOVE|END
   ...
   * Handle gestures
   * --------------------
   * inside the handler you can get/set lib.Gestures.detectionic.current. This is the current
   * detection sessionic. It has the following properties
   *      @param  {String}    name
   *      contains the name of the gesture we have detected. it has not a real function,
   *      only to check in other gestures if something is detected.
   *      like in the drag gesture we set it to 'drag' and in the swipe gesture we can
   *      check if the current gesture is 'drag' by accessing lib.Gestures.detectionic.current.name
   *
   *      readonly
   *      @param  {ionic.Gestures.Instance}    inst
   ...
   * the lib.Gestures.detectionic.current object is copied into lib.Gestures.detectionic.previous,
   ...
   * Register gestures
   * --------------------
   * When an gesture is added to the lib.Gestures.gestures object, it is auto registered
   * at the setup of the first lib.Gestures instance. You can also call lib.Gestures.detectionic.register
...
function tapClickGateKeeper(e) {
  ...
  // do not allow through any click events that were not created by lib.tap
...
(function(ionic) {
  ...
  ionic.Utils = {
    ...
    disconnectScope: function disconnectScope(scope) {
      ...
      scope.$broadcast('$lib.disconnectScope', scope);
    ...
    reconnectScope: function reconnectScope(scope) {
      ...
      scope.$broadcast('$lib.reconnectScope', scope);
  ...
  // Bind a few of the most useful functions to the lib scope
...
// Initialize the viewport height (after lib.keyboard.height has been
// defined).
 ...
 * for keyboard related events. Also aliased by lib.keyboard.enable.
 */
...
/**
 * Event handler for 'focusin' and 'lib.focusin' events. Initializes
...
function keyboardOrientationChange() {
  ...
  //console.log("orientation was: " + (lib.keyboard.isLandscape ? "landscape" : "portrait"));

  // toggle orientation
  ionic.keyboard.isLandscape = !ionic.keyboard.isLandscape;
  // //console.log("now orientation is: " + (lib.keyboard.isLandscape ? "landscape" : "portrait"));
...
function keyboardInitViewportHeight() {
  ...
  //console.log("lib.keyboard.isLandscape is: " + lib.keyboard.isLandscape);
...
function getViewportHeight() {
  ...
  //console.log('kb height is: ' + lib.keyboard.height);
  //console.log('kb isOpen: ' + lib.keyboard.isOpen);
  ...
  */
  ...
  __initEventHandlers: function() {
    ...
    self.mouseWheel = ionic.animationFrameThrottle(function(e) {
      var scrollParent = ionic.DomUtil.getParentOrSelfWithClass(e.target, 'lib-scroll');
          ...
          ionic.requestAnimationFrame(function(){
            ...
            if ( scrollTop > 0) {
              if (lib.Platform.isIOS()) {
                ...
                setTimeout(function(){
                  lib.tap.cloneFocusedInput(container, self);
                  self.scrollBy(0, scrollTop, true);
                  self.onScroll();
                }, 32);
              } else {
